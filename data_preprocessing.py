...
from torch_geometric.datasets import AttributedGraphDataset,WebKB,Twitch,LastFMAsia,CitationFull
import torch
...
class DataProcessor:
    def __init__(self, datapath, labelpath, dataset_name,random_state):
    ...
    def load_data(self):
        ...
        if self.dataset_name=='dblp':
            self.dataset = CitationFull(self.data_path, name=self.dataset_name)    
    ...
    def preprocess_data(self):
        ...
        self.coms = algorithms.wCommunity(self.G, min_bel_degree=0.28, threshold_bel_degree=0.28)
    ...
    def create_splits(self, test_size=0.25):
        label_data = pd.read_csv(self.label_path, header=None)
        label = label_data[0].tolist()
        
        train_label, test_label = train_test_split(label, test_size=0.25, random_state=self.random_state)
    ...
    def get_data_components(self):
        return self.g,  self.G, self.com_DICT, self.coms_G, self.edge_index, self.default_feat,self.node_features,self.number_class
